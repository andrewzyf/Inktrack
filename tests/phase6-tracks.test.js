...
describe('Starter tracks — land, sea and sky', () => {
  it('ships six tracks with distinct themes across three vehicle modes', () => {
    expect(STARTER_TRACKS.map((t) => t.id)).toEqual(['rooftop-run', 'ruin-rally', 'frost-peak', 'canyon-blitz', 'tidal-run', 'cloud-circuit']);
    const themes = STARTER_TRACKS.map((t) => getTheme(t.theme));
    expect(new Set(themes.map((t) => t.id)).size).toBe(6);
    expect(new Set(themes.map((t) => t.sky.horizon)).size).toBe(6);
    expect(Object.keys(THEMES)).toEqual(expect.arrayContaining(['rooftop', 'ruins', 'frost', 'canyon', 'ocean', 'sky']));
    const vehicles = STARTER_TRACKS.map((t) => buildTrack(t).vehicle);
    expect(vehicles).toEqual(['car', 'car', 'car', 'car', 'boat', 'plane']);
  });
  ...
  for (const track of STARTER_TRACKS) {
    describe(track.name, () => {
      ...
      it('has a full route, checkpoints, collectibles and its signature pieces', () => {
        ...
        expect(build.order.length).toBe(track.pieces.filter((p) => p.sc == null).length);
        expect(build.pots.length).toBeGreaterThanOrEqual(8);
        if (build.vehicle === 'plane') {
          expect(build.checkpoints.every((g) => g.ring)).toBe(true);
          expect(build.boostRings.length).toBeGreaterThan(2);
          expect(build.obstacles.length).toBeGreaterThan(5);
        } else {
          expect(types).toContain('ramp');
          expect(types.some((t) => t === 'bank' || t === 'curve')).toBe(true);
          expect(build.shortcutCount).toBeGreaterThanOrEqual(1);
        }
        if (build.vehicle === 'car') expect(types.some((t) => t === 'loop' || t === 'loopRight')).toBe(true);
      });

      it('stays inside the triangle budget', () => {
        expect(build.geo.triangleCount).toBeLessThan(160_000);
        expect(build.world.count).toBeLessThan(9000);
      });
      ...
      it('is completable start to finish (autopilot, no respawns)', () => {
        ...
        expect(race.finishTime).toBeGreaterThan(30);
